...
module.exports = function getGridAreas ({ zones, rowIndexes, colIndexes }) {
  const areaNames = []
  ...
  return indentMultiline(
    areaNames.map(
      (row) =>
        `"${row
          .map((name, x) =>
            (name + ' '.repeat(longestNameLengthByCol[x])).slice(
              0,
              longestNameLengthByCol[x]
            )
          )
          .join(' ')}"`
    ),
