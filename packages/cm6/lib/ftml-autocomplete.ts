...
const blockData: Record<BlockTypes, Record<string, TagSpec | null>> = {
  block: {
    ...
    'button': {
      ...
      keywords: [
        'set-tags', 'edit', 'edit-append', 'edit-sections',
        'history', 'print', 'files', 'tags', 'source', 'backlinks', 'talk',
        'delete', 'rename', 'site-tools', 'edit-meta', 'watchers', 'parent', 'lock-page'
      ]
    },
    // text
    'date': null,
...
for (const group in blockData) {
  ...
  if (group !== 'module') {
    ...
    blockAutocomplete.push(...Object.keys(specs).map((name): Completion => ({
      label: name,
      info: specs[name]?.info,
      type: 'type'
    })))
  } else {
    moduleAutoComplete.push(...Object.keys(specs).map((name): Completion => ({
      label: name,
      info: specs[name]?.info,
      type: 'class'
    })))
...
export function completeFTML(context: CompletionContext): CompletionResult | null {
  ...
  // Block node attribute values
    ...
    if (values) return {
      from: tree.name === 'string' ? tree.from + 1 : tree.from, to: pos, options: values.map(value => ({
        label: value,
        type: 'enum'
      }))
    }
